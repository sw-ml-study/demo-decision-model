...
impl<'a> Conversation<'a> {
    ...
    /// The part of a remembered line worth quoting back: from the memory
    /// trigger ("my") onward when it has one, as Weizenbaum's MEMORY rule did, so
    /// "well my mom never listens" comes back as "your mom never listens".
    fn statement(&self, text: &str) -> Vec<String> {
        let ws = words(text);
        let at = ws
            .iter()
            .position(|w| *w == self.script.memory.trigger)
            .unwrap_or(0);
        ws[at..].to_vec()
    }
