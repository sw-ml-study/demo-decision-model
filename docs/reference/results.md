...
A row is only written by a lesson's own gate script, never by hand. The `LB01`
row is the exception that proves the rule: it needs a local Ollama, so it is
produced by `scripts/llm-baseline` on demand, and every per-message row it used
is committed in `demos/typed-decisions/llm-baseline.tsv` so the summary can be
recomputed without re-running the models. The three
...
| DC01 | PR05 card scorer: score_i = f(h_state, h_question, h_card_i) over 55 cards of text, cosine with a learned scale, exact vocab 2,400, minibatch 256, 674 steps (5 min) | mlpl-repl 0.22.0 | 79,905 | 79,905 | 2,133,332 | 1 per card | ~20 (native, 51 cards) | val 85.3% on cards that trained; 0.4% full field / 36.5% five-card field on cards held out of training | - | - | - | - | - | -0.8 pts against model 4's fixed head on the rows it has columns for |
| TD01 | hello world: one 16-dim state, a Choice over 3 categories, a Noul and a 3-level Scale, exact vocab 720, 200 Adam steps on 72 messages | mlpl-repl 0.22.0 | 11,655 | 11,655 | 41,872 (bundled program) | 3 | ~3,000 (native, whole program) | test 0.750 category, 0.875 credentials, 0.708 urgency (24 held-out messages) | - | - | 0.426 raw, 0.404 cooled | 0.162 raw, 0.097 cooled | - | +37.5 pts over the majority class (0.375) on the same split |
| LB01 | baseline: local LLMs on TD01's 24 held-out messages, first-token top_logprobs renormalized over the legal answers, no generation | ollama 0.32.9 | 0.6B-31B (downloaded) | same | - | 1 | 170,000-1,240,000 | qwen3:0.6b 0.458 zero-shot / 0.500 few-shot; llama3.2:3b 0.792 / 0.750; gemma4:31b 1.000 | - | - | 0.687 / 0.739 / 0.326 / 0.293 / 0.015 | 0.110 / 0.268 / 0.006 / 0.080 / 0.021 | - | TD01 is 0.750: beaten by llama3.2:3b untrained, beats qwen3:0.6b |
| AT01 | demo 02: sw-atlas's 642-resource catalog, 386 frozen questions, 72 resources held out entirely; rerank scorer 77,985 params and intent+Noul model 17,351 | mlpl-repl 0.22.0 | 77,985 / 17,351 | same | - | 1 rerank / 3 typed | ~1,000,000 (250 steps, 17.5 min) | rerank over matcher top-20: 0.023 warm, 0.032 cold (random 0.050); intent 0.697 | - | - | - | - | - | matcher 0.233 warm / 0.209 cold accuracy@1, recall@20 0.63; intent baseline 0.737 |
