...
`demo-decision-model` builds a tiny non-generative **Typed Decision Model** in
sw-MLPL — the primitives **Choice**, **Noul**, and **Scale**, plus **Memory** —
and the Rust/Yew/WASM microscope that shows it deciding. ELIZA is **demo 01**, a
forcing function with a free deterministic oracle, not the subject.
...
Run **all** pre-commit checks including the mlplunit tests, commit to `main`
with a detailed message, push, then summarize what was pushed, the next
step(s), any choices needing a decision, and any blockers. This project commits
to `main` directly; the `feat/` to `pr/` handoff convention above does not apply
unless asked for.
